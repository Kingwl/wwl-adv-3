...
- 核心奖励选择第一版：第 1 关升级和宝箱奖励可基于 seed 生成 3 个不重复卡牌选项，并能把所选卡牌加入 run deck；触发奖励后会进入 reward 模式，选择奖励后才回到探索。
- 核心地牢地图第一版：地图 tile、关卡配置、run 状态、fixture 加载、移动阻挡、遭遇触发、拾取物收集和出口解锁。
- 核心 run 控制层第一版：`RunController` 统一玩家命令，供 UI 和流程测试共用移动、自动遭遇、出牌、结束回合、胜利清场、XP 掉落、奖励应用和出口请求。
...
- 奖励 UI：战斗胜利触发升级或探索拾取宝箱时显示 3 个卡牌奖励选项，支持鼠标 hover/点击、左右/A/D 切换和空格/回车确认，选择后加入牌组并回到探索。
...
1. 接入治疗、锻造等其他拾取物效果，并继续复用核心事件到 UI 的同步路径。
