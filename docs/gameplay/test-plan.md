...
- `game/test/godot/test_card_combat_core.gd` 覆盖 combo 严格按下一费续连、同费/低费/跳费重置、带 seed 的 deck 确定性、卡牌战斗中的伤害/block 结果、第一批敌人职责定义、关卡遭遇队列组合、敌人独立护甲吸收伤害、敌人护甲刷新、敌人多排队列、每排最多 5 个敌人、后排补位后延迟到下一轮攻击、单体攻击优先自动选择本轮最高攻击威胁的敌人，以及第 1 关奖励卡池的数量、唯一 id、中文显示名、攻击/防御/抽卡分布、前排、随机、弹跳、全体和复合效果结算。
